...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # --- Emeralds state ---
        em_ema = data.get("EMERALDS_EMA", 10000.0)
        ...
        if tom_symbol in state.order_depths:
            order_depth: OrderDepth = state.order_depths[tom_symbol]
            orders: List[Order] = []

            current_position = state.position.get(tom_symbol, 0)
            position_limit = 80
            ...
            if len(order_depth.buy_orders) > 0 and len(order_depth.sell_orders) > 0:
                ...
                if position_gap > 0:
                    ...
                    for ask_p, ask_v in sorted(order_depth.sell_orders.items()):
                        ...
                        if ask_p <= max_buy_price:
                            ...
                            if qty > 0:
                                orders.append(Order(tom_symbol, ask_p, qty))
                    ...
                    for bid_p, bid_v in sorted(order_depth.buy_orders.items(), reverse=True):
                        ...
                        if bid_p >= min_sell_price:
                            ...
                            if qty > 0:
                                orders.append(Order(tom_symbol, bid_p, -qty))
                                current_position -= qty
                                remaining -= qty
                ...
                if prim_buy > 0:
                    orders.append(Order(tom_symbol, maker_bid, prim_buy))
                if prim_sell > 0:
                    orders.append(Order(tom_symbol, maker_ask, -prim_sell))
                ...
                if sec_buy > 0:
                    orders.append(Order(tom_symbol, maker_bid - SEC_DEPTH, sec_buy))
                if sec_sell > 0:
                    orders.append(Order(tom_symbol, maker_ask + SEC_DEPTH, -sec_sell))

            result[tom_symbol] = orders
        ...
        if em_symbol in state.order_depths:
            em_depth: OrderDepth = state.order_depths[em_symbol]
            em_orders: List[Order] = []

            if len(em_depth.buy_orders) > 0 and len(em_depth.sell_orders) > 0:
                em_best_bid = max(em_depth.buy_orders.keys())
                em_best_ask = min(em_depth.sell_orders.keys())
                em_mid = (em_best_bid + em_best_ask) / 2.0

                alpha = 0.05
                em_ema = (alpha * em_mid) + ((1 - alpha) * em_ema)

            if abs(em_ema - 10000) <= 10:
                em_fair = 10000
            else:
                em_fair = int(round(em_ema))

            em_pos_limit = 80
            em_position = state.position.get(em_symbol, 0)

            acceptable_buy = em_fair - 1
            acceptable_sell = em_fair + 1

            if len(em_depth.sell_orders) != 0:
                for ask_price, ask_volume in sorted(em_depth.sell_orders.items()):
                    if ask_price <= acceptable_buy:
                        vol = min(abs(ask_volume), em_pos_limit - em_position)
                        if vol > 0:
                            em_orders.append(Order(em_symbol, ask_price, vol))
                            em_position += vol

            if len(em_depth.buy_orders) != 0:
                for bid_price, bid_volume in sorted(em_depth.buy_orders.items(), reverse=True):
                    if bid_price >= acceptable_sell:
                        vol = min(bid_volume, em_pos_limit + em_position)
                        if vol > 0:
                            em_orders.append(Order(em_symbol, bid_price, -vol))
                            em_position -= vol

            em_buy_cap = em_pos_limit - em_position
            em_sell_cap = em_pos_limit + em_position

            pos_skew = int(round(4.0 * em_position / em_pos_limit))
            em_maker_bid = em_fair - 6 - pos_skew
            em_maker_ask = em_fair + 6 - pos_skew

            if em_buy_cap > 0:
                em_orders.append(Order(em_symbol, em_maker_bid, em_buy_cap))

            if em_sell_cap > 0:
                em_orders.append(Order(em_symbol, em_maker_ask, -em_sell_cap))

            result[em_symbol] = em_orders
