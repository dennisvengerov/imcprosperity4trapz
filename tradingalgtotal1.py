...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ==========================================
        em_symbol = "EMERALDS"
        ema = stored_state.get("EMERALDS_EMA", 10000.0)

        if em_symbol in state.order_depths:
            em_order_depth: OrderDepth = state.order_depths[em_symbol]
            em_orders: List[Order] =[]
            
            # --- A. Calculate Mid Price & Update EMA ---
            if len(em_order_depth.buy_orders) > 0 and len(em_order_depth.sell_orders) > 0:
                best_bid = max(em_order_depth.buy_orders.keys())
                best_ask = min(em_order_depth.sell_orders.keys())
            ...
            position_limit = 80 
            current_position = state.position.get(em_symbol, 0)
            
            acceptable_buy_price = fair_price - 1
            acceptable_sell_price = fair_price + 1
            
            # --- D. "Taker" Strategy ---
            if len(em_order_depth.sell_orders) != 0:
                for ask_price, ask_volume in sorted(em_order_depth.sell_orders.items()):
                    if ask_price <= acceptable_buy_price:
                        trade_volume = min(abs(ask_volume), position_limit - current_position)
                        if trade_volume > 0:
                            em_orders.append(Order(em_symbol, ask_price, trade_volume))
                            current_position += trade_volume
                            
            if len(em_order_depth.buy_orders) != 0:
                for bid_price, bid_volume in sorted(em_order_depth.buy_orders.items(), reverse=True):
                    if bid_price >= acceptable_sell_price:
                        trade_volume = min(bid_volume, position_limit + current_position)
                        if trade_volume > 0:
                            em_orders.append(Order(em_symbol, bid_price, -trade_volume))
                            current_position -= trade_volume
            ...
            if buy_capacity > 0:
                ...
                em_orders.append(Order(em_symbol, maker_bid, buy_capacity))
                
            if sell_capacity > 0:
                maker_ask = fair_price + 6
                em_orders.append(Order(em_symbol, maker_ask, -sell_capacity))
        ...
        # 3. SAVE SHARED STATE
        # ==========================================
        stored_state["EMERALDS_EMA"] = ema
        traderData = json.dumps(stored_state)
