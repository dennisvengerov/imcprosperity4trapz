...
def load_clean_data():
    path = os.path.join(os.path.dirname(__file__), "ROUND1", "prices_round_1_day_*.csv")
...
def visualize_strategy_signals():
    df = load_clean_data()
    if df.empty:
        return
    ...
    fig.suptitle('IMC Prosperity - Quantitative Strategy Signals', fontsize=18, y=0.98)
    ...
    for i, product in enumerate(products):
        ...
        # Calculate Moving Averages for Crossover Strategy
        prod_df['SMA_50'] = prod_df['mid_price'].rolling(window=50, min_periods=1).mean()
        prod_df['SMA_500'] = prod_df['mid_price'].rolling(window=500, min_periods=1).mean()
        ...
        ax1.plot(prod_df['continuous_time'], prod_df['SMA_500'], color='red', linewidth=2, label='Slow SMA (500)')
        ax1.set_title(f'{product} - Trend & Crossovers', fontweight='bold')
        ax1.legend()
        ...
        # Ensure we only plot sensible spread ranges
        bins = np.arange(spreads.min(), spreads.max() + 2, 1) - 0.5
    ...
    output_path = os.path.join(os.path.dirname(__file__), "stock_visualizations_strategy.png")
    plt.savefig(output_path, dpi=300)
    print(f"Strategy Visualizations saved successfully to {output_path}")

if __name__ == "__main__":
    visualize_strategy_signals()